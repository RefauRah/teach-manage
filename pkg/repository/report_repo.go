...
func NewPostgresReportRepository(db *sqlx.DB) ReportRepository {
	return &PostgresReportRepository{db: db}
}

func (r *PostgresReportRepository) Create(report *model.Report) error {
...
func (r *PostgresReportRepository) Update(report *model.Report) error {
...
func (r *PostgresReportRepository) GetBySessionID(sessionID uuid.UUID, userID uuid.UUID) (*model.ReportResponse, error) {
...
func (r *PostgresReportRepository) GetByID(id uuid.UUID, userID uuid.UUID) (*model.ReportResponse, error) {
	var resp model.ReportResponse
	query := `SELECT r.id, r.session_id, r.material_taught, r.comprehension_score, r.comprehension_notes, r.homework, r.behavior_notes, r.recommendations, r.teacher_signature, r.parent_signature, r.created_at, r.updated_at,
	                 st.full_name AS student_name, su.name AS subject_name, se.session_date, se.start_time::text, se.end_time::text
	          FROM reports r
